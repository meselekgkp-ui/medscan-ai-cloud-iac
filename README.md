...
## 2. Offizielle Demo-Website

Die aktuelle IaC-basierte Demo-Website ist hier erreichbar:

```text
http://medical-xray-web-ayman-iac.s3-website-us-east-1.amazonaws.com/
```

Das statische Frontend liegt im Repository unter:

```text
frontend/index.html
```

Die Datei wurde manuell in den S3 Website Bucket des SAM/CloudFormation Stacks hochgeladen:

```text
medical-xray-web-ayman-iac
```

Der Website-Zugriff wurde mit HTTP Status Code `200` erfolgreich geprüft.
...
11. Das Frontend zeigt das Ergebnis aus DynamoDB an.
...
### 8.5 Keine internen Fehlermeldungen in API Responses
...
### 8.7 Keine echten Patientendaten
...
## 10. Zuverlässigkeit und Fehlerbehandlung

### 10.1 SQS-Entkopplung
...
### 10.7 Gradio Client Lazy Singleton
...
Das statische Frontend wird manuell in den S3 Website Bucket hochgeladen:

```powershell
aws s3 cp .\frontend\index.html s3://medical-xray-web-ayman-iac/index.html `
  --region us-east-1 `
...
```text
GitHub = Code, Frontend, Tests, Dokumentation, IaC
AWS = Laufende Cloud-Infrastruktur
SAM deploy = manuelle Brücke zwischen GitHub-Code und AWS-Umgebung
S3 upload = manuelle Veröffentlichung des statischen Frontends
```
...
## 19. Finales End-to-End-Testergebnis
...
### Frontend in S3 veröffentlichen
...
$response = Invoke-WebRequest `
  -Uri "http://medical-xray-web-ayman-iac.s3-website-us-east-1.amazonaws.com/" `
  -UseBasicParsing
...
try {
  Invoke-WebRequest `
    -Method POST `
    -Uri "https://3rhly6kvse.execute-api.us-east-1.amazonaws.com/prod/upload-url" `
...
## 22. Wichtige behobene Fehler

### 22.1 Falsche Logmeldung in GetMedicalResult
...
### 22.3 Presigned URL Signature Version
...
### 22.5 Gradio/Hugging Face Timeout
...
### 22.6 Gradio Client Performance
...
### 22.7 Processing Result Handling
...
### 22.8 Frontend war mit alter API verbunden

Problem:

Das ursprüngliche Frontend verwendete eine ältere API Gateway URL und fragte Ergebnisse mit `result?key=` ab.

Fix:

```text
API_BASE = https://3rhly6kvse.execute-api.us-east-1.amazonaws.com/prod
GET /result?id=<S3 object key>
```

Nutzen:

- Frontend und aktueller Backend-Stack passen zusammen
- End-to-End-Ablauf funktioniert über die IaC-Website

---

## 23. Was bewusst nicht umgesetzt wurde

### 23.1 Kein automatisches GitHub-to-AWS Deployment
...
### 23.5 Kein internes Modellhosting in AWS
...
## 24. AI-Assisted Review and Development Note
...
- Anpassung des Frontends an den aktuellen API Contract
...
S3 eignet sich für statisches Webhosting und Objektspeicherung. Das Frontend ist eine statische HTML-Datei, und Röntgenbilder sind objektbasierte Dateien.

### API Gateway

API Gateway stellt klare HTTP-Endpunkte für Frontend und Backend bereit.
...
SQS entkoppelt Upload und Verarbeitung. Dadurch wird die Architektur robuster, weil Upload und Analyse nicht direkt voneinander abhängig sind.
...
CloudWatch bietet Logs, Metriken, Dashboards und Alarme. Das ist wichtig, weil Serverless-Systeme ohne gutes Monitoring schwer nachvollziehbar sind.
...
Static Frontend in frontend/index.html ergänzt
Frontend an aktuellen API Contract angepasst
